...
import VideoMetaDataSkeleton from "../../components/skeleton/videoMetaDataSkeleton/VideoMetaDataSkeleton";
import VideoMetaData from "../../components/videoMetaData/VideoMetaData";
import {
  getRelatedVideos,
  getVideosById,
} from "../../redux/feature/videoSlice";
import { RelatedVideosState, SelectedVideoState } from "../../utils/types";
...
const WatchScreen = () => {
  ...
  const { selectedVideo, loading: relatedVideosLoading } = useSelector(
    (state: { selectedVideo: SelectedVideoState }) => state.selectedVideo
  );
  const { videos, loading } = useSelector(
    (state: { relatedVideos: RelatedVideosState }) => state.relatedVideos
  );
  const videoId = id || "";
  useEffect(() => {
    if (id) {
